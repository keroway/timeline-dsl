...
# Zigzag スタイル（レーン内アイテムを開始時刻順に上下交互に配置。単一/少数レーン向け。レーン数が 2 を超えるとエラー）
...
**対応機能:** `textDocument/publishDiagnostics` — パースエラーと検証警告を実際の行/列位置付きで通知。加えて `tdsl lint` の品質警告（`unused_lane` / `missing_id` / `invalid_calendar_date` 等）も、自動修正可能なものは WARNING、書き手の判断が必要なものは HINT として通知する。`textDocument/completion` — DSL キーワード補完候補を返す（文脈非依存・全キーワード）。`textDocument/hover` — lane ID にカーソルを当てるとラベル・kind・order を、QID にカーソルを当てるとキャッシュ済みエンティティ情報を表示（offline、ネットワーク不要）。`textDocument/definition` — lane 参照から宣言位置へジャンプ。`textDocument/references` — lane ID の全参照位置を返す。`textDocument/rename` / `prepareRename` — lane 宣言とその全参照を一括リネーム（明示的に `as <alias>` で宣言された lane のみ。slug 自動生成の lane は拒否）。`textDocument/documentSymbol` — timeline / lane / アイテムのアウトライン。`textDocument/codeAction` — `lint --fix` 相当の quick fix。`textDocument/formatting` — ソースの正準フォーマット（コメントは `tdsl fmt` と同様に保持されます。ブロック内部コメントは正準位置へ移動される場合があります）。
