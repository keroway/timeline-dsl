...
import type { WorkerMessage, WorkerRequest, WorkerSuccessResultMap } from './protocol'
...
function dispatchRequest(request: WorkerRequest): WorkerSuccessResultMap[typeof request.op] {
  switch (request.op) {
    case 'compileToIr':
    ...
    case 'renderSvgWithOptions': {
      ...
      return render_svg_from_source_with_options(source, scale, toJsRenderOptions(opts))
    }
    case 'renderHtml':
      return render_html_from_source(...request.args)
    case 'renderHtmlWithOptions': {
      const [source, opts = {}] = request.args
      return render_html_from_source_with_options(source, toJsRenderOptions(opts))
    }
    case 'checkSource':
...
async function main() {
  ...
  self.onmessage = (event: MessageEvent<WorkerRequest>) => {
    ...
    try {
      ...
      self.postMessage({ id: request.id, ok: true, result } satisfies WorkerMessage)
    } catch (err: unknown) {
      const error = err instanceof Error ? err.message : String(err)
      self.postMessage({ id: request.id, ok: false, error } satisfies WorkerMessage)
    }
  }
}
