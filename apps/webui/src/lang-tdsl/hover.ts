...
const KEYWORD_DOCS: Record<string, string> = {
  // Block keywords
  timeline:
    '年表全体を定義するトップレベルブロック。unit / range / calendar などのメタと各アイテムを内包する。',
  lane: 'アイテムを並べる横帯（レーン）を定義する。`lane "ラベル" as id { kind …; order …; }`。',
  group: '複数の lane をまとめるグループ。`group "ラベル" { lane … }`。',
  import:
    '外部データソース（Wikidata 等）からの取り込みブロック。`import wikidata as wd { … }`。',
  map: 'インポートしたエンティティを span / event / event_range に変換する。`map wd.alias to span { … }`。',
  template:
    '共通フォーマットを定義し apply で再利用する。`template "…" as id to span { … }`。',
  apply:
    'template を import に適用してアイテムを生成する。`apply <template> to <import> { lane …; }`。',
  color_map:
    'タグ→色のマッピングを宣言的に定義するブロック。`color_map { dynasty: "#4682B4"; }`。',
  policy:
    '再インポート時の挙動（merge_by_source / overwrite_imported / keep_manual）やフィールド優先度を指定する。',
  // Item keywords（lane・時間・ラベルは位置指定、続く { } は tags/source/id/origin のみ）
  span: '開始〜終了の期間を持つアイテム。`span <lane> <開始>..<終了> "ラベル" { … };`。',
  event:
    '単一時点のイベント。`event <lane> <時点> "ラベル" { … };`（時点は位置指定）。',
  event_range:
    '範囲を持つイベント。`event_range <lane> <開始>..<終了> "ラベル" { … };`。span と異なりイベント系の見た目になる。',
  // Misc / property / connective keywords
  as: '識別子に別名（エイリアス）を割り当てるキーワード。',
  to: 'map の変換先アイテム種別を指定する（`map … to span`）。',
  query:
    'SPARQL クエリで複数エンティティを一括インポートする。`query "…" as alias;`。',
  entity:
    '単一の Wikidata エンティティをインポートする。`entity Q… as alias;`。',
  ...
  source:
    'アイテムの出典。`source wd:Q…;`。インポート品は `wd:<QID>` が自動付与される。',
  label:
    'map ブロックの表示名式。`label label@ja ?? label@en;`。静的アイテムのラベルは位置指定の文字列リテラル。',
  start:
    'map ブロックで開始時点を計算する式。`start claim(P571).year;`（静的 span/event_range の時間は `開始..終了` の位置指定）。',
  ...
  field_priority:
    'フィールド別インポート優先度を定義する policy ブロック。`policy field_priority { label: manual; time: wikidata; tags: merge; }`。',
  origin:
    'アイテムの由来を表す block option。`origin imported;`（manual / imported）。',
}
...
function analyzeSource(src: string): SourceModel {
  ...
  for (const m of src.matchAll(
    /\bimport\s+([A-Za-z_][\w-]*)\s+as\s+([A-Za-z_][\w-]*)\s*\{/g
  )) {
    ...
    (view, pos): Tooltip | null => {
      ...
      if (query) {
        return mk(
          tipFrom,
          tipTo,
          makeTooltipDom('query', word, 'SPARQL クエリの結果セット')
        )
      }
      ...
      if (imp) {
        return mk(
          tipFrom,
          tipTo,
          makeTooltipDom('import', word, `インポート元: ${imp.sourceName}`)
        )
      }
