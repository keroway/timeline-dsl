...
- 種別は `Span` / `Event` / `Event range` のいずれか（#701: 周辺ツール群（`obsidian-tdsl` 等）のUI言語との一貫性のため既定は英語固定。以前は日本語ハードコードだった）
- ツールチップの改行は `, ` に変換されて 1 行に収められる
- `<title>` 要素はブラウザのツールチップ表示用のフォールバックとして残す
- **`locale` オプション**（#815、`tdsl-wasm` の `JsRenderOptions.locale`。CLI からは未公開）: `"en"`（既定）/ `"ja"` を選べる。影響するのは `Span:` / `Event:` / `Event range:` / `Lane:` という**構造プレフィックスのみ**で、タイトル・レーン名・年・id などソース由来の値はロケールに関係なく常にそのまま出力される。`"ja"` を指定すると `"スパン: 漢, BC206〜220, source: wd:Q7209, id: span:han, レーン: 漢"` のように `スパン:` / `イベント:` / `イベント範囲:` / `レーン:` になる
