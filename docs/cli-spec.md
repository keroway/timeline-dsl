...
| `--chart-pagination-range <N>` | タイムライン本体（チャート部分）を時間範囲軸で `N` ページに分割する（issue #733, ADR-0005 D3）。`meta.range` を `N` 個の連続する非空の整数年区間へ均等分割し、区間ごとに1ページを描画する（各ページの `meta` はサブ年精度フィールド〈`range_start_month` 等12個〉をクリアするため、月日/秒精度の `range` を持つ `.tdsl` でもページの目盛は整数年表示になる。元ファイルの宣言精度自体は変更されない）。区間境界をまたぐ `span`/`event_range` は既存のクランプ処理で各ページ側にクリップされ、`stderr` に `Warning: item "..." (...) is clipped at chart page boundary year(s) [...]; ...` を出力する（継続を示す視覚的マーカーは未実装、クリップされた事実のみを警告する）。現時点では `--format svg` のみ対応（`--format pdf` は明示エラー、統合は別途対応予定）。`--output <path>` は必須。`0` はエラー。`--watch` との併用、`--chart-pagination` との併用はいずれも明示エラー。`<stem>.pageN.<ext>` ごとに別ファイルとして書き出される点は `--chart-pagination` の SVG 出力と同じ | 無効（単一ページ） |
