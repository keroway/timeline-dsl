...
| `time` | `event` | 発生時刻 |
| `label` | ○ | 表示ラベル |
| `tags` | — | タグ（`|` 区切り + 可逆エスケープ。詳細は下記「`tags` 列のエンコーディング」参照） |
| `id` | — | アイテム ID（省略時は自動採番） |
...
完全一致するよう可逆エスケープを行います。

| 入力（タグ内の文字） | エスケープ後 | 備考 |
|---|---|---|
| `\`（バックスラッシュ） | `\\` | 常にエスケープ |
| `\|`（区切り文字自体） | `\|` | 常にエスケープ |
| タグの**先頭・末尾**の半角スペース | `\s` | `import-csv` の CSV reader は列全体の前後空白を除去するため、境界の空白は記号エスケープで保護する |
| タグの**先頭・末尾**のタブ / 改行 / 復帰 | `\t` / `\n` / `\r` | 同上 |
| 上記以外（タグ内側の空白、`,` など） | そのまま | エスケープ不要（`,` はもはや区切り文字として扱われない） |

`import-csv` はこのエスケープ規則に従って `tags` 列をデコードします。`\\`・`\|`・
`\s`・`\t`・`\n`・`\r` 以外の `\<char>` 形式（未知のエスケープ）や、末尾が
`\` で終わる不完全なエスケープは CSV 行番号付きのエラーとして拒否されます
（silent fallback 禁止）。

例: タグ配列 `["a|b", "c,d", " padded "]`（3タグ）は `tags` 列に
`a\|b|c,d|\spadded\s` としてエンコードされ、`import-csv` で同じ3タグへ復元されます。

**注意（#887 以前の挙動との違い）**: 従来 `import-csv` は `tags` 列を `|` と `,` の
両方で区切っていました（`,` を区切りとして扱うと、タグ自体に `,` を含む値との
区別がつかず無警告で分割されるバグ（#885）があったため）。この互換は廃止し、
`,` はタグ内の通常の文字として扱われます。手書き CSV で `,` 区切りのタグ列を
使っていた場合は、`|` 区切りへ書き換える必要があります（明示的な非互換）。
...
| `label` | 表示ラベル |
| `tags` | タグ（`|` 区切り + 可逆エスケープ。`import-csv` の「`tags` 列のエンコーディング」参照。#885） |
| `id` | アイテム ID |
