...
| `--pdf-pagination` | Split the `--show-table` item table across multiple pages sized to fit the paper size and margins (ADR-0004). Disabled by default (existing single-page scaled rendering is unchanged). Requires `--show-table`; an error otherwise. Only valid with `--format pdf`. See below for page layout when combined with `--chart-pagination` |
| `--chart-pagination <N>` | Split the timeline body (chart) into multiple pages by lane group, `N` lanes per page (issue #660/#661, ADR-0005 D2). Requires `--output`. Valid with both `--format svg` (multiple `<stem>.pageN.<ext>` files) and `--format pdf` (multiple pages within a single PDF file) |
...
- **Item table pagination** (`--pdf-pagination`): with `--format pdf` and `--show-table`, splits the output into a timeline page (page 1, as before) followed by table pages sized to fit the paper size and margins (ADR-0004). Each table page repeats the column headers and shows an `i / N` page footer, where `N` counts table pages only. Splitting the timeline body itself is out of scope for `--pdf-pagination` alone (ADR-0004 D1); see `--chart-pagination` below for combining both
- **Chart pagination** (`--chart-pagination <N>`): splits the timeline body into multiple pages by lane group, `N` lanes per page (issue #660/#661, ADR-0005 D2). The time axis (`meta.range`) is shared across pages, so span/event_range items are never clipped at a page boundary. `--show-legend` is drawn per chart page. A `lane.group` that is split across page boundaries prints a warning to stderr (never a silent no-op). `--output` is required
  - `--format svg`: written as separate `<stem>.pageN.<ext>` files (stdout is not supported). With `--show-table`, a single table page listing all items in the IR is appended after the chart pages (always `1 / 1`; multi-page tables are not supported in SVG)
  - `--format pdf` (issue #661): output as multiple pages within a single PDF file, ordered as chart pages (by lane group) followed by table pages. Without `--show-table` there are no table pages. `--show-table` alone (without `--pdf-pagination`) appends one unpaginated table page listing the full IR; combined with `--pdf-pagination`, table pages are generated by the existing row-splitting logic and their `i / N` footer counts table pages only. Existing `--format pdf` output without `--chart-pagination` is completely unchanged (ADR-0004 D3)
  - Combining with `--format html` or `--format png` is an error
