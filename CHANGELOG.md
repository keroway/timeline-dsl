...
- **LSP 補完の文脈を拡充し、値補完を追加**（#753）: (1) `detect_context` が `template` / `apply` を認識していなかったため、`template "..." as x to span {` の中で TopLevel 補完（`timeline`, `lane`, …）が出ていた。`template` の中身は `map_prop` と同一なので `Map` 候補を、`apply` は `apply_override` が `lane` のみを受理するので `lane` だけを出す。(2) item オプションの候補に **`note` / `link` / `color` が欠けていた**（文法・IR・`keywords.json` のハイライトには揃っているのに補完だけ無いという不整合）。lane の `color`（#747 で追加）も併せて出す。(3) **値の補完を新設**: `span` / `event` / `event_range` の直後は宣言済み lane ID、`map` の直後と `apply <template> to` の直後は import alias、`apply` の直後は template ID を `VALUE` kind で返す。編集途中の壊れたソースでも動くようパーサに通さない軽量スキャンで実装し、`as` を省略した宣言は ID を推測しない（間違った候補は打ち間違いを誘うため）。offline のみで Wikidata の entity key は対象外。WebUI 側の CodeMirror 補完にも同じ `note` / `link` / `color` の欠落があったので併せて修正した
- **WebUI の設定に `layout_style` / `show_legend` を露出**（#752）: `JsRenderOptions` は以前から両方を公開していたが、WebUI 側の `RenderOptions` 型・ワーカーの変換・設定モデルのいずれにも存在せず、`--layout-style group-bands / gantt / zigzag`（#543/#564/#565）と静的凡例（#544）が **CLI では使えるのに WebUI からは到達不能**だった（ギャラリーで `examples/grouped_dynasties.tdsl` を開いても group-bands 表示が見られなかった）。設定モーダルにレイアウト方式のセレクタと凡例パネルのトグルを追加し、再レンダリングの依存配列にも登録した。保存済み設定に未知の値が入っていた場合は他の `svg*` 設定と同じく既定へ戻す。**Rust 側の変更はゼロ**
- **item の `note` / `link` を出力側で活用する**（#749）: `note` / `link` は #566 / #567 で文法・IR・lowering・検証まで実装済みだったが、**出力でほぼ活きていなかった**（ツールチップに `link: https://…` とベタ書きされるだけでクリックできず、`grep data-note crates/tdsl-render/src` は 0 件だった）。SVG の item に `data-note` / `data-link` 属性を追加し、インタラクティブ HTML の詳細パネルに「メモ」「参照リンク」行を追加した（リンクは `target="_blank" rel="noopener noreferrer"` のアンカー）。**`link` を持つ item の SVG グループは `<a>` で包まない**: グループは既に `role="group"` と `tabindex="0"` を持つため、包むとフォーカス可能な要素が入れ子になり Tab が二重に止まる。加えて PNG / PDF は `usvg` を通るためアンカーは描画結果に現れない。リンクを押せる形で提供するのはインタラクティブ HTML の役割とし、SVG は `data-link` を載せるところまでとした（埋め込みページの JS からは読める）。`--show-table` への note 列追加は issue の指示どおりスコープ外
- **`lane` ブロックに `color` を追加し、lane 色を決定的にする**（#747）: レンダラは lane の並び順（`インデックス % パレット長`）から色を機械的に割り当てるため、**lane を 1 つ追加したり `order` を変えると既存 lane の色が全部ずれる**。バージョン管理下で長期運用する `.tdsl` では、この非決定性がそのまま図の非決定性になっていた。`lane "漢" as han { kind dynasty; order 20; color "#c0392b"; }` のように書くと固定できる。文法は item の `color_option` を再利用し（書式の二重定義を避けるため）、値の検証も item の `color` と同じ関数を使う。**不正値はエラー**（パレットへフォールバックすると打ち間違いが「なぜかこの色にならない」という形でしか現れない）。明示色には CSS 変数を被せない（変数は「パレットの N 番目」を上書きする仕組みで、意図して固定した色の差し替え経路にしない）。`IR` の `Lane.color` は `Option<String>` で `skip_serializing_if` 付きのため後方互換。`tdsl fmt` / `tdsl decompile` の往復でも保たれる
