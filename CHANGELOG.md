...
- **`osv-scan` CI が既知脆弱性で継続的に失敗していたのを修正**（#731）: `apps/webui` の `brace-expansion`（GHSA-mh99-v99m-4gvg / GHSA-rgw5-rvv9-x895、DoS）と `nanoid`（GHSA-2v37-7h3g-55p8、無限ループ）は `package.json` の `overrides` で修正版に固定した。`Cargo.lock` の `rustybuzz` 0.20.1（RUSTSEC-2026-0206）・`ttf-parser` 0.25.1（RUSTSEC-2026-0192）は本稿執筆時点で修正版が未リリースのため、`osv-scanner.toml` に理由を明記した上で一時的に ignore 対象とした（upstream リリース後に解除する）
